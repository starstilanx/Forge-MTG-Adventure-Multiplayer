...
public final class CMatchUI
    ...
    private final forge.screens.match.vfx.ParticleSystem vfxParticles =
            new forge.screens.match.vfx.ParticleSystem();
    private final forge.screens.match.vfx.VFXLayer vfxLayer =
            new forge.screens.match.vfx.VFXLayer();
    private forge.screens.match.vfx.VFXRenderer vfxRenderer;
    private forge.screens.match.vfx.VFXController vfxController;
    ...
    private void initMatch(final FCollectionView<PlayerView> sortedPlayers, final Collection<PlayerView> myPlayers) {
        ...
        final java.awt.Dimension displaySize = Singletons.getControl().getDisplaySize();
        Singletons.getView().getLpnDocument().add(vfxLayer, FView.TARGETING_LAYER, 0);
        vfxLayer.setSize(displaySize);
        vfxLayer.addComponentListener(new java.awt.event.ComponentAdapter() {
            @Override
            public void componentResized(final java.awt.event.ComponentEvent e) {
                if (vfxRenderer != null) {
                    vfxRenderer.resize(vfxLayer.getWidth(), vfxLayer.getHeight());
                }
            }
        });
        if (vfxRenderer == null) {
            vfxRenderer = new forge.screens.match.vfx.VFXRenderer(vfxParticles, displaySize.width, displaySize.height);
            vfxRenderer.setLayer(vfxLayer);
            vfxLayer.setRenderer(vfxRenderer);
            vfxRenderer.start();
        }
    ...
    public void openView(final TrackableCollection<PlayerView> myPlayers) {
        ...
        // Subscribe VFXController to the new game's event bus.
        vfxParticles.clear();
